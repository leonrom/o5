...
(function () {              // ---------------------------------------------- o5com/IniScripts ---
	...
	const
		...
		ScriptInit = e => {	//  завершение инициализации очередного скрипта
	...
	class Page {
		...
		ScriptsFinish = bytimer => { // конец инициализации страницы
			...
			this.scriptLoad.RemEvents(ScriptLoad)
			this.scriptDone.RemEvents(ScriptInit)
			// window.dispatchEvent(new window.Event('o5_isInited'))
			C.E.DispatchEvent('o5_isInited')
			window.olga5.C.o5_isInited = true
		}
		PageDones = e => { // закрытие всех новых элементов страницы
		...
		PageLoad = (e, second) => { // проверка и начало инициализации страницы !
			...
			if (C.consts.o5debug > 1 && e) {
				console.groupCollapsed('%c%s', this.clr, '____>  ' + e.type.padEnd(22) +
					(isOlga5Start ? 'ДА ' : '  ') + (isnew ? 'новая ' : 'повт. ') +
			...
			if (isnew && isloaded) {
				...
				// 					// window.addEventListener('o5inc_ready', e => this.PageLoad(e, true))	//1
				// 					C.E.AddEventListener('o5inc_ready', e => this.PageLoad(e, true))	//1
				// 					w.Init()
				// 				}
				// 				else
				...
				if (isOlga5Start) {	// есть теги с классом olga5Start
					...
					if (C.consts.o5timload) {
						...
						pact.timer = window.setTimeout(this.ScriptsFinish, 1000 * C.consts.o5timload, true)
					}

					this.scriptLoad.AddEvents(ScriptLoad)
					this.scriptDone.AddEvents(ScriptInit)

					this.pageDones.AddEvents(this.PageDones)

					this.errs.splice(0, this.errs.length)
					this.ScriptsStart()
					// InitScripts(`загружена страница '${url}'`)

					ScriptLoad()  // после InitScripts					
				}
				else
		...
		// HideEvents = e => { // проверка и начало инициализации страницы
		// 	const pact = this.pact,
		// 		url = DocURL()
		...
		constructor() {
			...
			const pageLoads = new MyEvents(C.consts.o5_pageLoads)
			pageLoads.AddEvents(this.PageLoad)	//{ capture: true }

			// const hideEvents = new MyEvents(C.consts.o5_pageHides)
			// hideEvents.AddEvents(this.HideEvents)	//{ capture: true }

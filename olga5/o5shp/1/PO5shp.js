...
(function () {              // ---------------------------------------------- o5shp/PO5shp ---11
    ...
    const
        ...
        ro = new ResizeObserver(entries => {
            for (const entry of entries) {
                console.log('Resized:', entry.target.id);
            }
        }),
        OnScroll = e => {
            console.log(e)
    ...
    class PO5 {
        ...
        constructor(tag) {
            ...
            nst = window.getComputedStyle(tag),
                Object.assign(pO5, {
                    ...
                    base: { pbase: null }, // (все pO5) ссылка на ближайший скроллируемый контейнер
            ...
            if (pO5.ibody) {
                ...
                console.log('как бы BODY', tag.id)
            }
            else
                if (pO5.scrls.H || pO5.scrls.V) {
                    ro.observe(tag)
                    tag.addEventListener('scroll', OnScroll)
                    console.log(tag.nodeName, tag.id)
                }
        ...
        actScroll;      // просто, чтобы ниже не ругались
        ActScroll(time) {

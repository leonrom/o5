...
/*jshint strict:true  */
/*jshint esversion: 6 */
function IsInitEvents() { // дублирование common.js
    "use strict"
...
function CC1(cb) {
    ...
    for (const shp of shps) { // тут нет вложенных - можно не заморачиваться с 
        ...
        if (cb) {
            if (pitch && pitch == cb) {
                cls.pitch = check3state[Check3state(pitch)]
                cb.title = (cls.pitch == 'P' ? "сталкивает" : (cls.pitch == 'S' ? "сдвигает" : "наезжает на")) + ' предыдущий'
            }
            if (alive && alive == cb) {

...
(function () {              // ---------------------------------------------- o5shp/DoChgs ---
	...
	const
		...
		CalcCovers = (pO5, ms) => {	//  пересчет въезжания вложенных контейнеров
			for (const i of [0, 1]) {  // ms[0] прямой ход, ms[1] - обратный
				const
					m = ms[i],
					isTL = 'TL'.includes(m)
		...
		makeScroll = {
			...
			РасфиксацияДругойСтороны: (aO5, x) => {
				...
				let chgo = false, j = pFixso.length
				while (j-- > 0) {
					const
						pFix = pFixso[j],
						vo = pFix.scops[o]
				...
				if (chgo && pFixso.length === 0) {
					...
					if (chgo || pFixso.find(p => p.bords.isChg) || fromTest)
			...
			ФиксацияПоX: (aO5, x) => {
				if (aO5.cls.puts.includes(x)) {
					...
					if (chgx || fromTest || pFixsx.find(p => p.bords.isChg))  // covers.act.isChg))
	...
	function MakeScroll(scV, scH, pcO5, fromTest) {
		...
		for (const x of xs) {
				...
				if (pO5.scops.isVisible)
					for (const m of ms) {
						...
						for (const aO5 of pBase.aO5s) {
							if (aO5.pFixs[m] !== pOut &&
								(d = IsOut(m, aO5.posC, vx)) > 0
							) {						// вышло за пределы видимости
								...
								if (aO5.pFixs[m] === pOut &&
									(d = IsOut(m, aO5.posO, vx)) < 0
								) { 				// снова стало видимым видимости
									aO5.UnFix(m, pOut)
								}
						}
					}
			...
			// 	makeScroll.РасфиксацияДругойСтороны(aO5, x)
			// 	makeScroll.ФиксацияПоX(aO5, x)

			// 	makeScroll.ОбрезПоГраницам(aO5, x)

			// 	if (aO5.act.fixed) {
			// 		for (const m of ms)
			// 			if (aO5.pFixs[m].length)
			// 				makeScroll.КоррекцияФиксированных(aO5, m)
